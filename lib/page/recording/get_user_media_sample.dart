...
class _GetUserMediaSampleState extends State<GetUserMediaSample> {
  ...
  void _startRecording() async {
    ...
    if (Platform.isIOS) {
      Log.d('Recording is not available on iOS');
      return;
    }
  ...
  void _toggleTorch() async {
    ...
    if (has) {
      Log.d('[TORCH] Current camera supports torch mode');
      setState(() => _isTorchOn = !_isTorchOn);
      await videoTrack.setTorch(_isTorchOn);
      Log.d('[TORCH] Torch state is now ${_isTorchOn ? 'on' : 'off'}');
    } else {
      Log.d('[TORCH] Current camera does not support torch mode');
    }
  }
